...
H1_drop = tf.layers.dropout(H1, drop_rate, training=training)
BN1 = tf.layers.batch_normalization(H1_drop, training=training, momentum=momentum)

H2 = tf.layers.dense(BN1, node_num, activation=tf.nn.elu,
                        kernel_initializer=He_init, name="H2")
H2_drop = tf.layers.dropout(H2, drop_rate, training=training)
BN2 = tf.layers.batch_normalization(H2_drop, training=training, momentum=momentum)

H3 = tf.layers.dense(BN2, node_num, activation=tf.nn.elu,
                        kernel_initializer=He_init, name="H3")
H3_drop = tf.layers.dropout(H3, drop_rate, training=training)
BN3 = tf.layers.batch_normalization(H3_drop, training=training, momentum=momentum)

H4 = tf.layers.dense(BN3, node_num, activation=tf.nn.elu,
                        kernel_initializer=He_init, name="H4")
H4_drop = tf.layers.dropout(H4, drop_rate, training=training)
BN4 = tf.layers.batch_normalization(H4_drop, training=training, momentum=momentum)

H5 = tf.layers.dense(BN4, node_num, activation=tf.nn.elu,
                        kernel_initializer=He_init, name="H5")
H5_drop = tf.layers.dropout(H5, drop_rate, training=training)
BN5 = tf.layers.batch_normalization(H5_drop, training=training, momentum=momentum)
...
for dropout_rate in dropout_rates :
    ...
    with tf.Session() as sess :
        ...
        for epoch in range(epoch_num) :
            ...
            for batch in range(train_batch_num) :
                sess.run([optimizer, BN_optimizer], feed_dict={training:True,
                            drop_rate:dropout_rate, momentum:best_momentum})
            ...
            epoch_loss, epoch_acc = sess.run([loss, accuracy],
                        feed_dict={drop_rate:0.0, momentum:best_momentum}) # kill the dropout for testing
        ...
        test_acc, test_correct, test_predict = sess.run([accuracy, correct,
                    predict], feed_dict={drop_rate:0.0, momentum:best_momentum}) # kill the dropout for testing
    ...
    plt.plot(epoch_axis, epoch_loss_axis, label='Training Loss, dropout rate='+str(dropout_rate))
    plt.plot(epoch_axis, valid_loss_axis, label='Validation Loss, dropout rate='+str(dropout_rate))
    plt.legend()
    plt.show()

    plt.plot(epoch_axis, epoch_acc_axis,
                label='Training Precision in percentage, dropout rate='+str(dropout_rate))
    plt.plot(epoch_axis, valid_acc_axis,
                label='Validation Precision in percentage, dropout rate='+str(dropout_rate))
    plt.legend()
    plt.show()
