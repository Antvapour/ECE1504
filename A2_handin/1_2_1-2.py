...
batch_X, batch_Y = iter.get_next(name="batch_xy")

# Construct a neural network
H1 = tf.layers.dense(batch_X, node_num, activation=tf.nn.elu,
                        kernel_initializer=He_init, name="H1")

H2 = tf.layers.dense(H1, node_num, activation=tf.nn.elu,
                        kernel_initializer=He_init, name="H2")

H3 = tf.layers.dense(H2, node_num, activation=tf.nn.elu,
                        kernel_initializer=He_init, name="H3")

H4 = tf.layers.dense(H3, node_num, activation=tf.nn.elu,
                        kernel_initializer=He_init, name="H4")

H5 = tf.layers.dense(H4, node_num, activation=tf.nn.elu,
                        kernel_initializer=He_init, name="H5")

Y_hat = tf.layers.dense(H5, output_num, name="Y_hat")
...
# Save the dataset initialization operation with name
...
with tf.Session() as sess :
    ...
    for epoch in range(epoch_num) :
        # train for one epoch
        sess.run(dataset_init_op, feed_dict={X:X_train1, Y:y_train1,
        ...
        sess.run(dataset_init_op, feed_dict={X:X_valid1, Y:y_valid1,
    ...
    sess.run(dataset_init_op, feed_dict={X:X_test1, Y:y_test1,
...
test_one_fail = np.argmin(test_correct)
plt.imshow(X_test1_pic[test_one_fail])
plt.show()
print("Ground truth is:", y_test1[test_one_fail])
print("Miss-predicted as:", test_predict[test_one_fail])
