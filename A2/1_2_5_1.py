...
# Iterate for different dropout rates
for dropout_rate in dropout_rates :
    ...
    with tf.Session() as sess :
        ...
        for epoch in range(epoch_num) :
            ...
            for batch in range(train_batch_num) :
                sess.run(optimizer, feed_dict={training:True,
            ...
            epoch_loss, epoch_acc = sess.run([loss, accuracy],
                        feed_dict={drop_rate:0.0}) # kill the dropout for testing
            epoch_acc = epoch_acc * 100

            print("Epoch =", epoch, "loss =", "{:.9f}".format(epoch_loss),
                    "accuracy =", "{:.2f}".format(epoch_acc), "per cent")
        ...
        test_acc, test_correct, test_predict = sess.run([accuracy, correct,
                    predict], feed_dict={drop_rate:0.0}) # kill the dropout for testing
    ...
    print("Final validation precision =", "{:.2f}".format(best_acc), "per cent")
